    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                // === Debug Mode: Uncomment the following lines to visualize depth values ===
                // Display IPC depth (normalized to 0-1 range for display)
                // return float4(gdepth / 100.0, gdepth / 100.0, gdepth / 100.0, 1);
                
                // Display Camera depth (normalized to 0-1 range for display)
                // return float4(cam_depth / 100.0, cam_depth / 100.0, cam_depth / 100.0, 1);
                
                // Display depth comparison result: green=IPC in front, red=Unity in front
                // return gdepth < cam_depth ? float4(0, 1, 0, 1) : float4(1, 0, 0, 1);
                
                // Display alpha value
                // return float4(gcol.a, gcol.a, gcol.a, 1);
                
                // Display IPC color (ignore depth and alpha)
                // return gcol;
