...
def loop_render(model_path, name, iteration, views, gaussians, pipeline, background, train_test_exp, separate_sh, fps):
    ...
    ipc_render.set_cam_list(width=width, height=height, views=views)
    try:
        while True:
            ...
            # Update IPC buffers
            ipc_render.update_frame(rendering_data, depth_data)

            # torchvision.utils.save_image(rendering_data, os.path.join(render_path, '{0:05d}'.format(view_idx) + ".png"))
            # torchvision.utils.save_image(depth_data, os.path.join(render_path, '{0:05d}_depth'.format(view_idx) + ".png"))
